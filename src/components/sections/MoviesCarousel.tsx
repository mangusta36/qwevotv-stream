...
export default function MoviesCarousel() {
  return (
    <section className="section-shell py-6 md:py-10">
      <div className="surface-panel overflow-hidden shadow-2xl">
        ...
        <div className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-4">
          {coverageItems.map((item) => {
            ...
            return (
              <div key={item.label} className="rounded-2xl border border-white/10 bg-slate-950 p-4 transition-colors hover:border-primary/40">
        ...
        <div className="grid gap-3 border-t border-white/10 p-5 md:grid-cols-3">
          {[
            { 
              src: "/images/live-sports.webp", 
              alt: "Live sports IPTV viewing on television", 
              label: "Live sports" 
            },
            { 
              src: "/images/fire-stick.webp", 
              alt: "Fire TV Stick setup", 
              label: "Fire TV setup" 
            },
            { 
              src: "/images/movies-series.webp", 
              alt: "Movies and series streaming", 
              label: "Movies and series" 
            },
          ].map((image) => (
            <div key={image.label} className="group relative aspect-[16/10] overflow-hidden rounded-2xl border border-white/10 bg-slate-950">
              <Image 
                src={image.src} 
                alt={image.alt} 
                fill 
